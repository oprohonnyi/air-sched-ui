...
class Timeline extends Component {
    ...
    parseFlightsToTimeline(flights) {
        ...
        for(let i = 0; i < flights.length; i++) {
            ...
            if(i !== 0 && flights[i].departuretime !== timeline[timeline.length - 1].to) {
                timeline.push({
                    state: 'idle',
                    from: timeline[timeline.length - 1].to + TURNAROUND_TIME,
                    to: flights[i].departuretime + TURNAROUND_TIME,
                    width: Math.floor((flights[i].departuretime - timeline[timeline.length - 1].to) / DAY_TIME_STEP),
                    id: null,
            ...
            timeline.push({
                ...
                width: Math.floor((flights[i].arrivaltime - flights[i].departuretime) / DAY_TIME_STEP),
                id: flights[i].id,
            ...
            if(i !== flights.length - 1) {
                ...
                timeline.push({
                    ...
                    to: DAY_TIME,
                    width: Math.floor((DAY_TIME - flights[i].arrivaltime) / DAY_TIME_STEP),
                    id: null,
    ...
    render() {
        ...
        return (
            <div id="timeline-wrapper">
                <div className="scale">
                    {timeline && timeline.map((frame, index) => 
                        <div
